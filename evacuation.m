...
Z0 = 50;        % nombre initial d'individus paniqués
Y0 = 50;        % nombre initial d'individus apeurés
...
while ti<tf
    ...
    for k=1:1:N
        ...
        if ((position(k,2)<=(yPorte+4)) && (position(k,1)<=(xPorte+lPorte/2+4)) && (position(j,1)>=(xPorte-lPorte/2-1)))
            Dp=Dp+1;
        end
    end
    Dp=Dp/(4*(lPorte+8));
