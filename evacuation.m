...
while ti<tf
    ...
    y=abs(position(:,2));
    % Individus sortis
    position(:,1) = x - sortis.*(x-xPorte);
    position(:,2) = y - sortis.*(y-yPorte);
    x=abs(position(:,1));
    y=abs(position(:,2));
    ...
    figure(4);
    %individus
    plot(x(calme),y(calme),'b.',x(peur),y(peur),'g.',x(panique),y(panique),'r.',x(sortis),y(sortis),'k.');
    % murs de la salle
    line([0 l],[L L],'Color','black');
    line([0 l],[0 0],'Color','black');
    line([l l],[0 L],'Color','black');
